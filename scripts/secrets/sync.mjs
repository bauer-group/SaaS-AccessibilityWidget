...
//   --push-only : ADD/OVERWRITE only (used by `pnpm secrets:push`).
...
//
// Usage:
//   pnpm secrets:push                  # add/overwrite from .env (no deletes)
//   pnpm secrets:sync                  # add/overwrite + delete (prompts)
//   pnpm secrets:sync -- --dry-run     # show plan, no changes
//   pnpm secrets:sync -- --yes         # apply without prompts (CI-safe)
...
if (values.help) {
  process.stdout.write(`Usage: pnpm secrets:sync [-- options]
