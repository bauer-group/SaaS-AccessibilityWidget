...
- **Rebrand auf den kanonischen Namen `@bauer-group/accessibility-widget`.** Der private Monorepo-Root heißt jetzt `@bauer-group/accessibility-widget-workspace`; `saas-` / `SaaS-`-Referenzen in package.json-URLs, README und Demo wurden auf `bauer-group/AccessibilityWidget` vereinheitlicht.
